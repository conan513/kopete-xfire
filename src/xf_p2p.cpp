...
void XfireP2P::slotSocketRead()
{
    ...
    QHostAddress sender;
    m_connection->readDatagram(datagram.data(), datagram.size(), &sender);
...
// CRC-32
static const quint32 _gfire_crc32_table[256] =
{
...
quint32 XfireP2P::calculateCrc32(const void *p_data, quint32 p_len)
{
	if(!p_data || !p_len)
		return 0;
	...
	quint32 i;
	for(i = 0; i < p_len; i++)
		crc32 = (crc32 >> 8) ^ _gfire_crc32_table[((quint8*)p_data)[i] ^ (crc32 & 0x000000FF)];

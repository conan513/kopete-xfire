...
XfireServer::XfireServer(XfireAccount *parent, const QString accountId, const QString accountPass, const QString serverName, const uint serverPort) : QObject(parent)
{
	...
	connect(m_connection, SIGNAL(disconnected()), m_account, SLOT(slotGoOffline()));
...
void XfireServer::slotConnected()
{
	...
	connect(m_connectionTimeout, SIGNAL(timeout()), this, SLOT(slotConnectionTimedOut()));
}
...
void XfireServer::socketRead()
{
	...
	while ((len = Xfire::Packet::requiredLen(m_buffer)) <= m_buffer.length())
	{
		...
		{
			kDebug() << "Received an invalid packet.";
...
void XfireServer::handlePacket(const Xfire::Packet *p_packet)
{
	...
	case 0x0082:
	{
		...
		if (!uid || !sid || uid->type() != Xfire::Attribute::Int32 ||
	...
	case 0x0085:
	{
		...
		if (!sid || sid->type() != Xfire::Attribute::SID ||
		...
		if (from != 0)
		{
			...
			case 1:
			{
				...
				from->setMessageSucceeded(); // FIXME: IM index support not implemented yet in setMessageSucceeded()
				break;
			}
			// P2P information
			/* case 2:
			{
				...
				if(m_account->mP2PConnection)
				{
					if(!from->session)
					{
						kDebug() << "Creating session.";
						from->session = new XfireP2PSession(from, salt->string());
						m_account->mP2PConnection->addSession(from->session);
					}

					// Set remote and local ip/port
					from->session->setRemoteAddress(ip->value(), port->value());
					from->session->setLocalAddress(localip->value(), localport->value());

					if(from->P2PRequested)
					{
						from->P2PRequested = FALSE;
						kDebug() << "Information received, sent handshake.";
					}
					else
					{
						QString localIP = m_connection->localAddress().toString();
						QString test = ""; // Ugly...

						sendP2pSession(from->mSession, QHostAddress(test).toIPv4Address(), m_account->mP2PConnection->m_connection->localPort(),
									   QHostAddress(localIP).toIPv4Address(), m_account->mP2PConnection->m_connection->localPort(), 4, salt->string());

						from->P2PRequested = TRUE;
						kDebug() << "Request received, sent own data.";
					}
				}
				else
				{
					sendP2pSession(from->mSession, 0, 0, 0, 0, 0, salt->string());
					from->P2PRequested = TRUE;

					kDebug() << "Received P2P request, denied.";
				}
...
void XfireServer::slotConnectionTimedOut()
{
	kDebug() << "Connection timed out";
